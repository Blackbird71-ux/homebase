...
```bash
# SSH to NAS, then:
cp /path/to/data/homebase.db /path/to/data/backups/homebase-pre-wipe-$(date +%Y%m%d-%H%M%S).db

docker compose exec app sqlite3 /data/homebase.db << 'EOF'
...
-- Reset bill paid/invoice status (keep the bill templates themselves)
...
-- Reset income received status (keep income entry templates)
