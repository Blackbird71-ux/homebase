...
- **New hook** managing all layout state and interaction logic:
  ...
  - **Auto-positioning:** Cards without saved layouts are placed in alternating columns (0% or 52%) with `y = row * 35`, `width: 48`, `height: 'auto'`.
  - **Persistence:** Debounced 500ms save via `onSave` callback.
- **File:** `src/lib/hooks/useCardLayout.ts` (NEW — 469 lines)
...
- **File:** `src/components/dashboard/DashboardGrid.tsx` (MODIFIED — 165 lines)
...
- **File:** `src/app/(app)/home/HomeClient.tsx` (MODIFIED — 122 lines)
...
- **Auto-positioning:** New cards (not yet persisted) are placed left/right alternating, row by row, at default positions
