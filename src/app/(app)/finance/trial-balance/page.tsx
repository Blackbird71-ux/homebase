...
export default function TrialBalancePage() {
  ...
  function exportCSV() {
    ...
    if (data.mode === 'trial-balance') {
      csv = 'GL Code,Account Name,Type,Parent,Debit,Credit,Net Balance\n'
      for (const a of data.accounts) {
        csv += `"${a.glCode ?? ''}","${a.name}","${a.type}","${a.parentName ?? ''}",${a.totalDebit},${a.totalCredit},${a.netBalance}\n`
      }
      csv += `\n"","TOTAL","","",${data.grandTotalDebit},${data.grandTotalCredit},${data.grandTotalDebit - data.grandTotalCredit}\n`
    } else if (data.mode === 'general-ledger') {
      csv = 'Date,Reference,Description,Type,Debit,Credit,Balance\n'
      csv += `"Opening Balance","","","",,,${data.openingBalance}\n`
      for (const l of data.lines) {
        csv += `"${format(new Date(l.date), 'd/MM/yyyy')}","${l.reference}","${l.description}","${l.type}",${l.debit || ''},${l.credit || ''},${l.balance}\n`
      }
    }
    ...
    a.href     = url
    a.download = data.mode === 'trial-balance' ? 'trial-balance.csv' : `gl-${data.glAccount.name.replace(/\s+/g, '-')}.csv`
    a.click()
