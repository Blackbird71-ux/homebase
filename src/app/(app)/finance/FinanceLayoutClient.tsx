...
function AdminNavItem({ pathname, mobile = false }: { pathname: string; mobile?: boolean }) {
  const isActive = pathname.startsWith('/finance/admin')
  if (mobile) {
    return (
      <div className="px-3 py-2 border-t border-border mt-1">
        ...
        >
          <ShieldAlert className={cn('h-4 w-4 shrink-0', isActive ? 'text-primary' : 'text-muted-foreground/60')} />
  ...
  return (
    <div className="px-2 pb-3 mt-auto border-t border-border pt-3 shrink-0">
      <Link
        href="/finance/admin"
        className={cn(
      ...
      >
        <ShieldAlert className={cn('h-3.5 w-3.5 shrink-0', isActive ? 'text-primary' : 'text-muted-foreground/70')} />
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 right-0 z-30 bg-background border-b border-border shadow-lg overflow-y-auto max-h-[70svh]">
            ...
            {isAdmin && <AdminNavItem pathname={pathname} mobile />}
          </div>
        </>
      )}
    </div>
...
function DesktopSidebar({ pathname, draftCount, isAdmin, groups }: { pathname: string; draftCount: number; isAdmin: boolean; groups: NavGroup[] }) {
  return (
    <aside className="hidden md:flex w-52 shrink-0 flex-col border-r border-border bg-muted/30 overflow-y-auto">
      ...
      {isAdmin && <AdminNavItem pathname={pathname} />}
    </aside>
  )
}
