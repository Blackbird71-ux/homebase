...
// Creates a contact from a Family.birthdays entry and removes that entry,
// so the birthday's single source of truth becomes the contact.
export async function POST(req: Request) {
  ...
  const { contact, reusedExisting } = await convertBirthdayToContact(user.familyId, name, date)
